  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        buttonLink,
        imageUrl,
        mobileImageUrl: mobileImageUrl || imageUrl, // fallback if mobile image is not provided
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-lg font-bold text-slate-800 font-headline">Hero Banners</h2>
          <p className="text-xs text-slate-500 font-medium">Manage slideshow slides for the homepage hero section</p>
        </div>
        <button
          ...
          <div className="bg-white rounded-xl max-w-3xl w-full shadow-2xl border border-slate-200 overflow-hidden my-8 animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center px-6 py-4 bg-slate-50 border-b border-slate-200">
              <h3 className="font-bold text-slate-800">
                {editId ? 'Edit Hero Banner' : 'Create New Hero Banner'}
              </h3>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
              ...
              <div>
                <label className="text-xs font-semibold text-slate-600 uppercase block mb-1">Banner Subtitle / Description</label>
                <textarea
                  value={subtitle}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="text-xs font-semibold text-slate-600 uppercase block mb-1">Primary Button Text</label>
                  <input
                    type="text"
                    value={buttonText}
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-600 uppercase block mb-1">Primary Button URL</label>
                  <input
                    type="text"
                    value={buttonLink}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-slate-50 border border-slate-200 rounded-lg">
                <div>
                  <label className="text-xs font-semibold text-slate-700 uppercase block mb-2">Desktop Banner Image (1920x1080) *</label>
                  <CloudinaryUpload 
                    value={imageUrl} 
                    onChange={setImageUrl} 
                    folder="hero_banners_desktop"
                  />
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-700 uppercase block mb-2">Mobile Banner Image (1080x1920)</label>
                  <CloudinaryUpload 
                    value={mobileImageUrl} 
                    onChange={setMobileImageUrl} 
                    folder="hero_banners_mobile"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="text-xs font-semibold text-slate-600 uppercase block mb-1">Sort Order</label>
                  <input
                    type="number"
                    value={sortOrder}
